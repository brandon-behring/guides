...
- **Series roadmap v2 (read first if resuming)**: `docs/plans/active/2026-06-10_series_roadmap_v2.md` — canonical roadmap for the company-agnostic, interview-prep-led AI-native series; consolidates the 06-03 roadmap + 06-04 handoff + 06-08 audit (all now in `done/`). Guides #1 (Evaluation) + #2 (LLM App Engineering) both complete + reviewed (13 ch each) in the sibling repo `~/guides-ai-engineering` (its `docs/RESUME.md` continues the build). **Site LIVE 2026-06-12**: hub at `guides.brandon-behring.dev` (custom domain on the `guides-hub` Worker); guides standalone at `guides-ai-engineering.brandon-m-behring.workers.dev` until the `/ai-engineering/*` path proxy ships (blocked on scaffold #140/#141). Remaining launch work: roadmap §5 path proxy + L2 (OG/CI/analytics). Demand framing: `docs/research/role_demand_and_interview_signals/`.
