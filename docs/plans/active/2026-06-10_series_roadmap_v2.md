...
| **Guide #3 — Production AI systems** | **COMPLETE + independently reviewed (2026-06-13).** All 13 ch (0–12) in the §5 shape — production loop (deploy/serve/observe/evaluate/respond) + judgment + capstone + craft; 4 computed islands + 7 ScenarioQuiz. 3-agent review, all findings fixed (`~/guides-ai-engineering/docs/REVIEW_FINDINGS_2026-06-13.md`); anchors bijective. Pushed through `7ea858d`. |
| Companion (`companion/`, stdlib-only) | `mini_eval` 6 modules **24/24** · `mini_rag` 5 modules (search·chunk·pipeline·rerank·budget) **29/29** · `mini_agent` 3 modules (loop·tools·orchestrate+crew) **12/12** · `mini_prod` 5 modules (latency·cascade·trace·monitor·drift, builds on `mini_rag.budget`+`mini_eval`) **44/44**. |
