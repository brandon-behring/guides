# Cross-pollination review — P1 research-lint + P2 pedagogy-playbook wrap-up
...
**Scope:** Study what `guides` can learn from a sibling project by the same author on the same scaffold (a private repo; the comparison memo is kept local) → rank the adoptions → implement the two near-term, hub-only picks (P1 research-lint CI, P2 pedagogy playbook).
**Session duration:** ~1 focused session

## TL;DR

A side-by-side read of the sibling project produced a ranked cross-pollination memo (P1–P4; kept local — it studies a private repo in detail). The two near-term picks were then built:
...
|---|---|---|
| Cross-pollination memo (kept local; not committed — see Scope) | ✅ | 21 referenced paths confirmed; framing honest (guides ahead on research rigor; bidirectional) |
...
- **P2 = distillation, not new research.** Read design v0.3 in full + the handoff + style guide; the decision log is a consolidation, every row sourced. Visual principles re-grounded for interview-prep (not copied from the sibling project).
